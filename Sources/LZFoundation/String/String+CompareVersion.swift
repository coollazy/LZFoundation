public extension String {
    ...
    func isLessThen(version: String) -> Bool {
        guard isSameVersionType(version: version) else {
            return false
        }
        let currentVersions = split(separator: ".").map({ String($0) })
    ...
    func isEqualTo(version: String) -> Bool {
        guard isSameVersionType(version: version) else {
            return false
        }
        let currentVersions = split(separator: ".").map({ String($0) })
    ...
    // 判斷是否為相同格式的版本號
    func isSameVersionType(version: String) -> Bool {
...
extension Array where Element == String {
    fileprivate func isGreaterThan(versions: [String]) -> Bool {
        ...
        for (key, value) in enumerated() {
            if value.intValue > versions[key].intValue {
                return true
            } else if  value.intValue < versions[key].intValue {
                return false
            }
        }
        return false
