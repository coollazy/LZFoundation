...
public extension Date {
    ...
    func isEqualTo(date: Date, format: String = "yyyy-MM-dd HH:mm:ss") -> Bool {
        return formatter(format: format) == date.formatter(format: format)
    }
    
    // 判斷是否大於 Date ，format 可設定判斷的日期格式
    func isGreaterThan(date: Date, format: String = "yyyy-MM-dd HH:mm:ss") -> Bool {
        return formatter(format: format) > date.formatter(format: format)
    }
    
    // 判斷是否小於 Date，format 可設定判斷的日期格式
    func isLessThan(date: Date, format: String = "yyyy-MM-dd HH:mm:ss") -> Bool {
        return formatter(format: format) < date.formatter(format: format)
    }
}
